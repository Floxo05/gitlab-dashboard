...
export default async function GroupMRsAnalysisPage(props: { params: Promise<{ full_path: string }> }) {
  const params = await props.params;
  const fullPath = String(params.full_path || "");

  const currentTarget = `/analysis/group/${fullPath}/merge-requests`;
  ...
  return (
    <div className="mx-auto max-w-6xl p-6 space-y-6">
      <header className="space-y-1">
        ...
        <h1 className="text-2xl font-semibold">MR-Durchsatz: Übersicht (Platzhalter)</h1>
      ...
      <section className="rounded-md border border-gray-200 p-4">
        <div className="text-sm text-gray-600">Demnächst: Erstellte vs. gemergte MRs, Durchlaufzeiten, Review-Dauer, WIP.</div>
      </section>
    </div>
  );
}
